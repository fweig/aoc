...
#define idx2add(i0, i1)                                                        \
	(idx2)                                                                 \
	{                                                                      \
		.x = (i0).x + (i1).x, .y = (i0).y + (i1).y                     \
	}
#define idx2sub(i0, i1)                                                        \
	(idx2)                                                                 \
	{                                                                      \
		.x = (i0).x - (i1).x, .y = (i0).y - (i1).y                     \
	}
#define idx2eq(i0, i1) ((i0).x == (i1).x && (i0).y == (i1).y)
...
#define at2(arr2d, idx) at(at(arr2d, (idx).y), (idx).x)
#define height(arr2d) ((arr2d).size)
#define width(arr2d) ((height(arr2d) == 0) ? 0 : first(arr2d).size)
#define inside(arr2d, idx)                                                     \
	((idx).x >= 0 && (idx).y >= 0 && (idx).x < (i64)width(arr2d) &&        \
	 (idx).y < (i64)height(arr2d))
...
#define aforeachr(array, iter, offset, xsize)                                  \
	for ((iter) = &at(array, offset);                                      \
	     (iter) < (array).d + (xsize < 0 ? (array).size :                  \
					       (size_t)((xsize) + (offset)));  \
	     (iter)++)

#define aforeach2d_idx(array, pos)                                             \
	for ((pos).y = 0; (pos).y < (i64)height(array); (pos).y++)             \
		for ((pos).x = 0; (pos).x < (i64)width(array); (pos).x++)
...
#define amap_parallel(iarr, oarr, block)                                       \
	do {                                                                   \
		req(empty(oarr));                                              \
		aresize(oarr, (iarr).size);                                    \
		size_t _i;                                                     \
		_Pragma("omp parallel for schedule(dynamic)")                  \
		for (_i = 0; _i < (iarr).size; _i++) {                         \
			vtype(iarr) *it = &at(iarr, _i);                       \
			vtype(oarr) val = (block);                             \
			at(oarr, _i) = val;                                    \
		}                                                              \
	} while (0)

#define amap2d(iarr, oarr, block)                                              \
	do {                                                                   \
		req(empty(oarr));                                              \
		vtype(iarr) *_row;                                             \
		aforeach(iarr, _row) {                                         \
...
#define amap_str(iarr, oarr, block)                                            \
	do {                                                                   \
		req(empty(oarr));                                              \
		vtype(iarr) *_row;                                             \
		aforeach(iarr, _row) {                                         \
...
#define areducer(array, offset, size, neutral, block)                          \
	({                                                                     \
		__typeof__(array) _arr = (array);                              \
		req(!empty(_arr));                                             \
...
#define asumr(array, offset, size)                                             \
	areducer(array, (offset), (size), 0, { a + b; })
...
void yput(void *ptr)
{
	if (H.size + 1 > H.capacity) {
		...
		H.capacity = MAX(H.capacity, 8ul);
		H.d = realloc(H.d, H.capacity * sizeof(void *));
	}
	H.d[H.size++] = ptr;
}
