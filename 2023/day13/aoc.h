...
#define foreach(a) for (auto *it = a.data(); it != end(a); it++)

#define foreachr(array, offset, xsize)                                         \
  for (auto *it = &array[offset];                                              \
       it != (array).data() +                                                  \
                 (xsize < 0 ? (array).size() : (i64)((xsize) + (offset)));     \
       it++)

#define foreach2d(array, pos)                                                  \
...
int main(int argc, char *argv[])
{
  ...
  if (!testenv("NOTASK1")) {
    ...
    ilog(">>> Task1 Result: %ld", r);
  }
  yfree();
  if (!testenv("NOTASK2")) {
    ilog(">>> Task2");
    i64 r = task2(input_file);
    ilog(">>> Task2 Result: %ld", r);
  }
  yfree();
  return 0;
}
...
void *yrealloc(void *oldptr, size_t bytes)
{
  ...
  for (auto *it = H.data; it != H.data + H.size; it++) {
    if (*it == oldptr)
      *it = newptr;
  }
  return newptr;
